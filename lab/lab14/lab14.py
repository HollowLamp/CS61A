...
def prune_min(t):
    ...
    if t.is_leaf():
        return None
    min_branch = t.branches[0] if t.branches[0].label < t.branches[1].label else t.branches[1]
...
def num_splits(s, d):
    ...
    def helper(s, sum1, sum2):
        ...
        s1 = s[0]
        return helper(s[1:], sum1 + s1, sum2) + helper(s[1:], sum1, sum2 + s1)
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            pre = helper_align(skeleton_idx + 1, code_idx + 1)
            match, cost = skel_char + pre[0], pre[1]
            possibilities.append((match, cost))
        # Insert
        pre = helper_align(skeleton_idx, code_idx + 1)
        match, cost = "+[" + code_char + "]" + pre[0], pre[1] + 1
        possibilities.append((match, cost))
        # Delete
        pre = helper_align(skeleton_idx + 1, code_idx)
        match, cost = "-[" + skel_char + "]" + pre[0], pre[1] + 1
        possibilities.append((match, cost))
...
def filterl(lst, pred):
    ...
    """
    "*** YOUR CODE HERE ***"
    def combine(filtered, ele):
        if pred(ele):
            return Link(ele, filtered)
        else:
            return filtered
    return reverse(foldl(lst, combine, Link.empty))
...
#相当于右折叠并不累积值，而是累积要计算的函数，之后通过调用这个函数来实现反向,可以类比成栈
#延迟操作执行，之后进行一次性处理
