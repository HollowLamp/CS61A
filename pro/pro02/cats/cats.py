...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    tmp = [x for x in paragraphs if select(x)]
    if k  >= len(tmp):
        return ''
    else:
        return tmp[k]
...
def accuracy(typed, reference):
    ...
    for i in range(0, min(len(typed_words), len(reference_words))):
        if typed_words[i] == reference_words[i]:
            counts += 1
    return counts / len(typed_words) * 100 if len(typed_words) != 0 else 0.0
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    "*** YOUR CODE HERE ***"
    words = len(typed) / 5
    return words * 60 / elapsed if elapsed <= 60 else words / (elapsed / 60)
    # END PROBLEM 4
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        return user_word
    ans = user_word
    tmp = diff_function(user_word, valid_words[0], limit)
    if tmp < limit:
        ans = valid_words[0]
    for x in valid_words[0:]:
        if tmp > diff_function(user_word, x, limit):
            tmp = diff_function(user_word, x ,limit)
            if tmp <= limit:
                ans = x
    return ans 
    # END PROBLEM 5
...
def time_per_word(times_per_player, words):
    ...
    "*** YOUR CODE HERE ***"
    # 使用官方solution
    # 创建每一个player的时间列表，列表*数字表示创建一个列表，包含原列表元素的n个重复, 加上for循环代表重复创建指定次
...
def fastest_words(game):
    ...
    for wordindex in word_indices:
        #当前单词
        word = word_at(game, wordindex)
        #每个玩家的对特定单词的时间列表
        timelst = []
        for id in player_indices:
            #将每个玩家对于这个单词的时间列表存储
            timelst += [time(game, id, wordindex)]
        #用时最小值对应的索引就是最快的玩家id
        idindex = timelst.index(min(timelst))
        #将最快的单词存入对应id的列表
        ans[idindex].append(word)
        #+= 可以添加多个元素，append只能添加一次
    return ans
    # END PROBLEM 10
