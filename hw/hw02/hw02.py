...
def missing_digits(n):
    ...
    if n//10 == 0:
        return 0
    if n//10%10 < n%10-1:
        return 1+missing_digits(n//10*10+n%10-1)
...
def count_coins(total):
    ...
    def helper(res, min):
        ...
        if res < min:
            return 0
        return helper(res-min, min)+helper(res,next_largest_coin(min))
...
def make_anonymous_factorial():
    ...
    True
    """
    #最神奇的一集，这个答案纯纯是我走投无路了干脆把本身作为参数传入。这道题确实是just4fun，实际不会这么写，看看就好
