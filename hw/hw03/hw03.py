...
def balanced(m):
    ...
    if is_planet(m):
        ...
        lend = end(left(m))
        rend = end(right(m))
        lt = length(left(m)) * total_weight(lend)
        rt = length(right(m)) * total_weight(rend)
        return balanced(lend) and balanced(rend) and lt == rt
...
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        branch = []
        branch.append(totals_tree(end(left(m))))
        branch.append(totals_tree(end(right(m))))
...
def replace_leaf(t, find_value, replace_value):
    ...
    if label(t) == find_value and is_leaf(t):
        ans = tree(replace_value)
        return ans 
    br = []
    for branch in branches(t):
        br.append(replace_leaf(branch, find_value, replace_value))
...
def sub_interval(x, y):
    ...
    and any value in y."""
    "*** YOUR CODE HERE ***"
    negay = interval(-upper_bound(y), -lower_bound(y))
    return add_interval(x, negay)
...
def quadratic(x, a, b, c):
    ...
    "最值点不在区间内，要多比较一个最值点"
    return interval(min(f(upper_bound(x)), f(lower_bound(x)), f(extreme_point)), max(f(upper_bound(x)), f(lower_bound(x)), f(extreme_point)))
